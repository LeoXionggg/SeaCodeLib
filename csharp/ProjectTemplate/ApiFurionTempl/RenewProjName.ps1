...
If([String]::IsNullOrEmpty($NewName))
{
	Write-Host '未输入新项目名称，已退出！' -ForegroundColor Red
	exit
}
...
Write-Host ' ===内容替换[' -NoNewLine -ForegroundColor Yellow
Write-Host $OldName -NoNewLine
Write-Host ']->[' -NoNewLine -ForegroundColor Yellow
Write-Host $NewName -NoNewLine
Write-Host ']处理开始=== ' -ForegroundColor Yellow
...
Start-Sleep -s 1
Write-Host ' ===文件更名[' -NoNewLine -ForegroundColor Yellow
Write-Host $OldName -NoNewLine
Write-Host ']->[' -NoNewLine -ForegroundColor Yellow
Write-Host $NewName -NoNewLine
Write-Host ']处理开始=== ' -ForegroundColor Yellow
